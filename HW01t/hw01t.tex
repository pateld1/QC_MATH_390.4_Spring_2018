...
\usepackage{graphicx}
\graphicspath{{/Users/darshanpatel/Desktop/LaTex Notes/Math390.4/Homeworks/HW01t/}}
...
\emph{Data} is defined to be the normal \qu{measured} result of a phenomenon. 
...
\emph{Predictions} is defined to be a probable outcome to a certain situation. 
...
\qu{All models are wrong ...} because they're not reality. 
...
\qu{All models are wrong but some are useful} because they serve as some useful function even though it's not reality. 

\easysubproblem{What is the difference between a "good model" and a "bad model"?}\spc{2} \\
A \qu{good model} is one that creates predictions that are close to the data. A \qu{bad model} is one where the predictions created is far from the data. 
...
\\ This model is \qu{bad} because it does not take into consideration of other factors that can keep the doctor away or closer. An apple a day may have no role at all. 
...
\easysubproblem{Why is \emph{supervised learning} a \emph{empirical solution} and not an \emph{analytic solution}?}\spc{3} \\
...
An example of $g$ is: $$g(x_i) = \indicator{x_i > 1} $$ 
